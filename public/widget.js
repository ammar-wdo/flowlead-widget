...
(function() {
  ...
  function createChatWidget() {
    ...
    iframe.src = `https://flowlead-widget.vercel.app/${companySlug}/widget`;
    iframe.style.position = 'fixed';
    iframe.style.bottom = window.innerWidth < 600 ? '3px' : '20px';
    iframe.style.right = window.innerWidth < 600 ? '3px' : '20px';
    iframe.style.width = '150px';
    iframe.style.height = '50px';
    iframe.style.border = 'none';
    iframe.style.zIndex = '1000';
    ...
    // Append the iframe to the widget container only when it's ready
    const widgetContainer = document.createElement('div');
    widgetContainer.id = 'widget-container';
    document.body.appendChild(widgetContainer);

    window.addEventListener('message', (event) => {
      if (event.data === 'widget-ready') {
        widgetContainer.appendChild(iframe);
      } else if (event.data === 'open-widget') {
        setIframeSize(iframe);
        iframe.style.borderRadius = '25px';
      } else if (event.data === 'close-widget') {
        iframe.style.width = '150px';
